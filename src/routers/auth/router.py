...
from .service import (TokenScheme,
                      ...
                      create_new_user,
                      get_all_users_from_db, banning_user, unbanning_user, refresh_token)
...
@router.post(AUTHORIZATION_URL, response_model=TokenScheme,
             description='Authorize user and create access and refresh token')
async def authorization_user(jwt_token: Annotated[TokenScheme, Depends(verity_user_and_make_token)]):
    return jwt_token


@router.post(REFRESH_TOKEN_URL, response_model=TokenScheme, response_model_exclude_none=True,
...
@router.patch(BAN_USER_URL, response_model=UserReadSchema, description='Ban user (Only for admin!)')
async def ban_user(banned_user: Annotated[UserReadSchema, Depends(banning_user)]):
    return banned_user


@router.patch(UNBAN_USER_URL, response_model=UserReadSchema, description='Unban user (Only for admin!)')
async def ban_user(ubbanned_user: Annotated[UserReadSchema, Depends(unbanning_user)]):
    return ubbanned_user


@router.delete(DELETE_USER_URL, response_model=UserReadSchema, description='Delete user by id (Only for admin!)')
